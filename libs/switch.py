## {{{ http://code.activestate.com/recipes/577507/ (r2)
##
...
class Switch(object):
	...
	# find_pipes_with_same_start_end a match, then if the switch value is a string, the 'in' case
	# matches are tried next, in the order they were registered.  If that
	# doesn't find_pipes_with_same_start_end a match, then if the switch value is a string,
	# the regular-expression case matches are tried next, in
	# the order they were registered.  If that doesn't find_pipes_with_same_start_end a match, and
	...
	def switch(self, switchValue, *args, **kwargs):
		...
		try:
			# Can we find_pipes_with_same_start_end an exact match for this switch value?
...
# kwargs: {}"
## end of http://code.activestate.com/recipes/577507/ }}}
